import express, { Request, Response } from 'express';
import { qdrant } from '../config/qdrantClient';
import { getClipImageEmbedding } from '../services/imageToImageService';
import multer from 'multer';

// Configure Multer to keep uploaded files in RAM (Buffer) instead of saving to disk
const upload = multer({ storage: multer.memoryStorage() });

const router = express.Router();

// Insert record into image_to_image_collection
router.post('/image-to-image/records', upload.any(), async (req: Request, res: Response) => {
  try {
    const { id, label, imageUrl } = req.body;

    // upload.any() puts all uploaded files into an array called req.files
    const files = req.files as Express.Multer.File[];
    const uploadedFile = files && files.length > 0 ? files[0] : null;

    // 1. Determine the source: Use the uploaded file Buffer if it exists, otherwise use the URL
    const imageSource = uploadedFile ? uploadedFile.buffer : imageUrl;

    if (!imageSource) {
      return res.status(400).json({ error: "Please provide either an 'image' file upload or an 'imageUrl' string." });
    }

    // Convert the form-data string into a strict Base-10 integer
    const numericId: number = Number.parseInt(id, 10);

    if (Number.isNaN(numericId)) {
      return res.status(400).json({ error: "The 'id' field must be a valid number." });
    }

    // Process via Sharp -> RawImage -> CLIP Embedding
    const vector = await getClipImageEmbedding(imageSource);

    await qdrant.upsert('image_to_image_collection', {
      wait: true,
      points: [{ id: numericId, vector, payload: { label, imageUrl: uploadedFile ? 'local_upload' : imageUrl  } }],
    });

    res.json({ message: `Image-to-Image record ${id} inserted.` });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});

// Query Image by another Image
router.post('/image-to-image/search', upload.any(), async (req: Request, res: Response) => {
  try {
    const { searchImageUrl } = req.body;

    // upload.any() puts all uploaded files into an array called req.files
    const files = req.files as Express.Multer.File[];
    const uploadedFile = files && files.length > 0 ? files[0] : null;

    // 1. Determine the source: Use the uploaded file Buffer if it exists, otherwise use the URL
    const imageSource = uploadedFile ? uploadedFile.buffer : searchImageUrl;

    if (!imageSource) {
      return res.status(400).json({ error: "Please provide either an 'image' file upload or an 'imageUrl' string." });
    }

    // Process input image via Sharp -> RawImage -> CLIP Vector
    const queryVector = await getClipImageEmbedding(imageSource);

    const results = await qdrant.search('image_to_image_collection', {
      vector: queryVector,
      limit: 3,
      with_payload: true,
    });

    res.json({ searchImageUrl, results });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});

export default router;